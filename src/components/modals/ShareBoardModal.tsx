...
export default function ShareBoardModal({
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Invited ${email} to board`);
      setEmail('');
      onMembersUpdated();
    } catch (err: any) {
  ...
  const handleConfirmRemoveMember = async () => {
    ...
    try {
      ...
      toast.success(`Removed ${memberToRemove.user?.name || 'member'} from board`);
      onMembersUpdated();
  ...
  return (
    <>
      ...
      {/* Modern Confirm Remove Member Modal */}
      <ConfirmDeleteModal
        isOpen={!!memberToRemove}
        onClose={() => setMemberToRemove(null)}
        onConfirm={handleConfirmRemoveMember}
        title="Remove Member"
        description={`Are you sure you want to remove ${memberToRemove?.user?.name || 'this member'} (${memberToRemove?.user?.email}) from "${board.name}"? They will lose access to this board.`}
